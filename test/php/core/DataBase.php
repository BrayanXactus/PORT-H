    ...
    class DataBase 
    {
        ...
        public function __construct() 
        {
            try {
                ...
                $this->pdo = new PDO("dblib:host=$hostname;dbname=$dbname;charset=utf8","$username","$password");
                //$this->pdo = new PDO("sqlsrv:server=DB_HOST,DB_PORT;Database=DB_DATA",DB_USER,DB_PASSWORD);
        ...
        public function execQuery(): bool 
        {
            try {
                ...
                /*$this->stm->execute($this->parsingValuesQuery($this->aExecute));
                var_dump($this->sSql);
                print_r($this->aExecute);
                
                var_dump($this->stm->execute($this->parsingValuesQuery($this->aExecute)));
        ...
        public function insertMultiple(string $sTable, array $aRows): bool 
        {
            try {
                ...
                $this->oSistema->logErrores($e);
            } catch (Throwable $ex) {
                $this->oSistema->logErrores($ex);
            }
        ...
        /*
        * @description : restar días, meses, años u horas a una fecha
        */
        ...
        public function restarFechas(string $sFecha, int $iDia = 0, int $iMes = 0, $iAnio = 0, int $iHora = 0, int $iMinuto = 0, int $iSegundos = 0) 
        {
            ...
            if ($iSegundos > 0) {
                $date->modify("-" . $iSegundos . " second");
                $sNuevaFecha = $date->format("Y-m-d H:i:s");
            }

            if ($iMinuto > 0) {
                $date->modify("-" . $iMinuto . " minute");
                $sNuevaFecha = $date->format("Y-m-d H:i:s");
            }
            
            if ($iHora > 0) {
                $date->modify("-" . $iHora . " hour");
                $sNuevaFecha = $date->format("Y-m-d H:i:s");
            }
            
            if ($iDia > 0) {
                $date->modify("-" . $iDia . " days");
                $sNuevaFecha = $date->format("Y-m-d H:i:s");
            }
            
            if ($iMes > 0) {
                $date->modify("-" . $iMes . " months");
                $sNuevaFecha = $date->format("Y-m-d H:i:s");
            }
            
            if ($iAnio > 0) {
                $date->modify("-" . $iAnio . " years");
                $sNuevaFecha = $date->format("Y-m-d H:i:s");
            }
